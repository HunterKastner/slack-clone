...
export const getById = query({
  ...
  handler: async (ctx, args) => {
    ...
    // If no user ID is found, you can either return null or an empty workspace instead of throwing an error
    if (!userId) {
      // You might want to return a null response or an appropriate message for unauthenticated access
      return null; // or return an empty object or some default value
    }
    ...
    // If no membership found, return null or an appropriate response
    if (!member) {
      return null; // or handle with more specific error message depending on your needs
    }

    // Fetch and return the workspace details if the user is a member
